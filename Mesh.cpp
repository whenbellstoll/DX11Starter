...
Mesh::Mesh( Vertex v[], int vIndex, unsigned int indices[], int iIndex, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	...
	ibd.ByteWidth = sizeof(unsigned int) * iIndex;	// iIndex number of indices in the buffer
...
void Mesh::CreateBuffers(Vertex* v, int vIndex, unsigned int* indices, int iIndex, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	...
	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex) * vIndex;      // number of vertices in the buffer
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER; // Tells DirectX this is a vertex buffer
	vbd.CPUAccessFlags = 0;
	vbd.MiscFlags = 0;
	vbd.StructureByteStride = 0;
	...
	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(unsigned int) * iIndex;	// number of indices in the buffer
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;	// Tells DirectX this is an index buffer
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	ibd.StructureByteStride = 0;
